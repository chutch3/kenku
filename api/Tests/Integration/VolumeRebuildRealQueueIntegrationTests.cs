...
/// <summary>
/// Like <see cref="VolumeRebuildIntegrationTests"/>, but driven by the REAL <see cref="WorkerQueue"/>
/// instead of the harness's sequential runner — so the unbundle→rebundle ordering is enforced by the
/// production dependency scheduler (dependsOn), not by enqueue order. The correct final bundle can only
/// be produced if unbundle actually runs before rebundle.
/// </summary>
[Trait("Category", "Integration")]
public class VolumeRebuildRealQueueIntegrationTests : IDisposable
{
    private readonly IntegrationHarness _harness = new("%M - Ch.%C");
    private readonly WorkerQueue _queue;

    public VolumeRebuildRealQueueIntegrationTests() =>
    ...
    [Fact]
    public async Task RealQueue_RebuildsStaleBundle_OrderingUnbundleBeforeRebundle()
    {
