...
public record QueuedJob(string Key, string Type, JobStatus Status, JobFailureKind FailureKind, int Attempts,
    int MaxAttempts, string? ResourceKey, string? Error, DateTime CreatedAt, DateTime ScheduledFor,
    DateTime? StartedAt, DateTime? FinishedAt, string? Progress, string Payload)
    : Identifiable(Key)
{
    ...
    [Required] [Description("Why a parked job is parked — NeedsChoice means it awaits a user pick, not a retry.")]
    public JobFailureKind FailureKind { get; init; } = FailureKind;
    ...
    public static QueuedJob From(JobEntity job) => new(job.Key, job.Type, job.Status, job.FailureKind, job.Attempts,
        job.MaxAttempts, job.ResourceKey, job.Error, job.CreatedAt, job.ScheduledFor, job.StartedAt, job.FinishedAt,
        job.Progress, job.Payload);
}
